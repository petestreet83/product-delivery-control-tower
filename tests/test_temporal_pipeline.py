...
class TemporalPipelineTests(unittest.TestCase):
    ...
    def test_history_and_delta_fall_back_to_rollups(self):
        policy = RetentionPolicy(rollup_after_days=1, raw_retention_days=2)
        store = TemporalStore(self.db_path, retention_policy=policy)
        now = datetime.now(timezone.utc)
        old_ts_1 = (now - timedelta(days=3, hours=1)).isoformat()
        old_ts_2 = (now - timedelta(days=3)).isoformat()
        store.insert_points(
            [
                NormalizedMetric(
                    source="open-meteo",
                    ...
                    value=18.0,
                    unit="celsius",
                    event_timestamp=old_ts_1,
                    ingest_timestamp=old_ts_1,
                    source_version="v1",
                ),
                NormalizedMetric(
                    source="open-meteo",
                    ...
                    value=22.0,
                    unit="celsius",
                    event_timestamp=old_ts_2,
                    ingest_timestamp=old_ts_2,
                    source_version="v1",
                ),
            ]
        )
        ...
        self.assertEqual("rollup", history[0]["source_version"])
